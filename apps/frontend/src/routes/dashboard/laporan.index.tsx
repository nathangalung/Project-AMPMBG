...
function LaporanPage() {
  ...
  const getRiskStyle = (level: string) => {
    ...
    // Tambahkan fallback '' agar tidak error jika level null
...
function FilterSection({ onFilter }: FilterSectionProps) {
  const [localFilters, setLocalFilters] = useState<FilterValues>({
    startDate: "", endDate: "", provinceId: "", cityId: "", category: "", status: ""
  })
  ...
  const handleReset = () => {
      const empty: FilterValues = { startDate: "", endDate: "", provinceId: "", cityId: "", category: "", status: "" }
    ...
    <div className="bg-general-20 rounded-xl p-4 md:p-6 shadow-sm border border-general-30 mb-6">
      <div className="flex flex-col sm:flex-row justify-between sm:items-center mb-4 gap-2">
        ...
        <div className="flex gap-4 items-center flex-wrap">
            {error && <div className="flex items-center gap-2 text-red-100 text-xs bg-red-20 px-3 py-1.5 rounded-full border border-red-30 font-medium"><AlertCircle className="w-3.5 h-3.5" />{error}</div>}
            {Object.values(localFilters).some(v => v !== "") && (
                 <button onClick={handleReset} className="flex items-center gap-1 text-red-100 hover:text-red-90 text-xs font-medium transition-colors ml-auto sm:ml-0"><XCircle className="w-3.5 h-3.5" /> Reset Filter</button>
            )}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-6 gap-4">
        <div className="lg:col-span-2">
          ...
          <input type="date" value={localFilters.startDate} min={minDate} max={today} onChange={(e) => handleDateInput('startDate', e.target.value)} className={inputClass} />
        </div>
        <div className="lg:col-span-2">
          <label className={labelClass}>Akhir</label>
          <input type="date" value={localFilters.endDate} min={localFilters.startDate || minDate} max={today} onChange={(e) => handleDateInput('endDate', e.target.value)} className={inputClass} />
        </div>
        <div className="lg:col-span-2">
          <label className={labelClass}>Status</label>
          <div className="relative">
            <select value={localFilters.status} onChange={(e) => handleChange('status', e.target.value)} className={`${inputClass} appearance-none cursor-pointer truncate pr-8`}>
        ...
        <div className="lg:col-span-2">
          ...
          <div className="relative">
            <select value={localFilters.provinceId} onChange={(e) => handleChange('provinceId', e.target.value)} className={`${inputClass} appearance-none cursor-pointer truncate pr-8`}>
        ...
        <div className="lg:col-span-2">
          ...
          <div className="relative">
            <select value={localFilters.cityId} onChange={(e) => handleChange('cityId', e.target.value)} disabled={!localFilters.provinceId} className={`${inputClass} appearance-none cursor-pointer truncate pr-8 disabled:bg-general-30/30`}>
              <option value="">Semua</option>
        ...
        <div className="lg:col-span-2">
          ...
          <div className="relative">
            <select value={localFilters.category} onChange={(e) => handleChange('category', e.target.value)} className={`${inputClass} appearance-none cursor-pointer truncate pr-8`}>
              <option value="">Semua</option>
        ...
        <div className="lg:col-span-6 flex justify-end mt-2">
          <button type="button" onClick={handleSearch} className="w-full sm:w-auto px-6 py-2.5 bg-blue-100 hover:bg-blue-90 text-general-20 rounded-lg transition-all flex items-center justify-center gap-2 body-sm font-heading font-medium shadow-sm hover:shadow active:scale-[0.98]">
