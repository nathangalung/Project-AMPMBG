...
kitchenNeeds.post("/requests", authMiddleware, zValidator("json", submitRequestSchema), async (c) => {
  ...
  if (!kitchenNeed) return c.json({ error: "Konten kebutuhan dapur tidak ditemukan" }, 404)
...
kitchenNeeds.post("/admin/upload", rateLimiter(10, 60 * 60 * 1000), adminMiddleware, async (c) => {
  ...
  return c.json({ data: { imageUrl: url }, message: "Gambar berhasil diunggah" })
})
...
kitchenNeeds.post("/admin", adminMiddleware, zValidator("json", createKitchenNeedSchema), async (c) => {
  ...
  return c.json({ data: item, message: "Konten berhasil dibuat" }, 201)
})
